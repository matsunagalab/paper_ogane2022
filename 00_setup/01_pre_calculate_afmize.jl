...
function pre_calculate_afmize()
    ...
    for radius in radii
        for imodel in 1:1:nmodel
            command = `mkdir -p ../data/calc_afms/radius_$(radius)/sharpness_$(sharpness)/q_$(nq)/model_$(imodel)`
            run(command)
        end
    end
    ...
    Threads.@threads for imodel in 1:nmodel
        ...
        for iq in 1:nq
            ...
            for iparam in 1:nparam
                if isfile("../data/calc_afms/radius_$(radii[iparam])/sharpness_$(sharpness)/q_$(nq)/model_$(imodel)/$(iq).bson")
                    continue
                end
                param = params[iparam]
                calculated = MDToolbox.afmize(model_rotated, param)
                @save "../data/calc_afms/radius_$(radii[iparam])/sharpness_$(sharpness)/q_$(nq)/model_$(imodel)/$(iq).bson" calculated
            end
        end
    end
end
