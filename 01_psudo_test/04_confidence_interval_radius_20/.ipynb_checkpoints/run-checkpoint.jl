...
for (itest, init_q) in enumerate(extracted_qs)
    ...
    @save "arg_id_$(arg_id)_iq_$(init_q)_likelihood.bson" params r
end

rs = []
for init_q in extracted_qs
    @load "arg_id_$(arg_id)_iq_$(init_q)_likelihood.bson" params r
    push!(rs, r)
end
...
@save "arg_id_$(arg_id)_baumwelch.bson" T_true T learned_T learned_pi_i obs_list log_emission ids nid
