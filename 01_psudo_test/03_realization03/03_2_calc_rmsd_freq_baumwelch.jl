...
function calc_rmsd_freq_baumwelch()
    ...
    for pred_radius in pred_radii
        run(`mkdir -p ../data/viterbi/test_radius_$(test_radius)_realization03/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)`)
        rs = []
        ...
        @save "../data/viterbi/test_radius_$(test_radius)_realization03/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/rmsds_baumwelch.bson" rmsds_arr
        @save "../data/viterbi/test_radius_$(test_radius)_realization03/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/T_baumwelch.bson" T learned_T learned_pi_i obs_list log_emission ids nid
    end 
end
