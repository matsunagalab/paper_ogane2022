...
function calc_rmsd_freq_baumwelch()
    ...
    for pred_radius in pred_radii
        run(`mkdir -p ../data/viterbi/test_radius_$(test_radius)_realization02/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)`)
        rs = []
        for iq in extracted_qs
            @load "../data/result/test_radius_$(test_radius)_realization02/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/iq_$(iq)_noise_$(sigma_noise)_nframe_$(nframe).bson" params r
            push!(rs, r)
        end
        ...
        for (itest, iq) in enumerate(extracted_qs)
            @load "../data/test_case/radius_$(test_radius)_realization02/sharpness_$(sharpness)/q_$(nq)/iq_$(iq)_noise_$(sigma_noise)_nframe_$(nframe).bson" afms param imodel_array dxdy_array
            @load "../data/result/test_radius_$(test_radius)_realization02/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/iq_$(iq)_noise_$(sigma_noise)_nframe_$(nframe).bson" params r
        ...
        @save "../data/viterbi/test_radius_$(test_radius)_realization02/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/rmsds_baumwelch.bson" rmsds_arr
        @save "../data/viterbi/test_radius_$(test_radius)_realization02/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/T_baumwelch.bson" T learned_T learned_pi_i obs_list log_emission ids nid
    end 
end
