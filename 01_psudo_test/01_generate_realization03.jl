...
run(`mkdir -p data/test_case/radius_$(probe_radius)_realization03/sharpness_$(sharpness)/q_$(nq)`)
...
Threads.@threads for init_q in 1:nq
    ...
    @save "data/test_case/radius_$(probe_radius)_realization03/sharpness_$(sharpness)/q_$(nq)/iq_$(init_q)_noise_$(sigma_noise)_nframe_$(nframe).bson" afms param imodel_array dxdy_array
end