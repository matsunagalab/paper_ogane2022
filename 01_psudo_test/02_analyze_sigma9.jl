...
function analyze()
    ...
    for pred_radius in pred_radii
        run(`mkdir -p data/result/test_radius_$(test_radius)_sigma_$(sigma_noise)/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)`)
        ...
        for iq in 1:nq
            @load "data/test_case/radius_$(test_radius)_sigma_$(sigma_noise)/sharpness_$(sharpness)/q_$(nq)/iq_$(iq)_noise_$(sigma_noise)_nframe_$(nframe).bson" afms

            # all models with parallel map
            r = getposterior_parallel_use_local_afm(models, afms, qs, sharpness, params)

            @save "data/result/test_radius_$(test_radius)_sigma_$(sigma_noise)/pred_radius_$(pred_radius)/sharpness_$(sharpness)/q_$(nq)/iq_$(iq)_noise_$(sigma_noise)_nframe_$(nframe).bson" params r
        end
    end
end
